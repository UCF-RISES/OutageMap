...
for j in weatherEvents.index:
    ...
    for i in nodes.index:
        ...
        try:
            ...
            timeframe = getWeatherByCoords(long, lat, begin, end)
        timeframe[i] = np.sqrt(np.square(timeframe["wind_u"]) + np.square(timeframe["wind_v"]))
        print(timeframe[i])
        eventForNode.append(timeframe[i] * 2.23694)

    events = pd.DataFrame(eventForNode)
    pd.DataFrame.to_csv(events, f'../P3R/Wind/nodes/weatherEvent{j+1}.csv')
